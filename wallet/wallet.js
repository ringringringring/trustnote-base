...
var objectHash = require('../crypto/objectHash.js');
var ecdsaSig = require('../crypto/signature.js');
var network = require('../p2p/network.js');
var storage = require('../data/storage.js');
var device = require('./device.js');
var walletGeneral = require('./walletGeneral.js');
var lightWallet = require('./lightWallet.js');
var walletDefinedByKeys = require('./walletDefinedByKeys.js');
var walletDefinedByAddresses = require('./walletDefinedByAddresses.js');
var eventBus = require('../common/eventBus.js');
var ValidationUtils = require("../validation/validationUtils.js");
var composer = require('../unit/composer.js');
var indivisibleAsset = require('../token/indivisibleAsset.js');
var divisibleAsset = require('../token/divisibleAsset.js');
var profiler = require('../common/profiler.js');
var breadcrumbs = require('../common/breadcrumbs.js');
...
function sendMultiPayment(opts, handleResult)
{
	...
	readFundedAndSigningAddresses(
		...
		function(arrFundedAddresses, arrBaseFundedAddresses, arrAllSigningAddresses){
			...
			// if we have any output with text addresses / not trustnote addresses (e.g. email) - generate new addresses and return them
...
function sendTextcoinEmail(email, subject, amount, asset, mnemonic){
	...
	replaceInTextcoinTemplate({amount: amount, asset: asset, mnemonic: mnemonic, usd_amount_str: usd_amount_str}, function(html, text){
		mail.sendmail({
			to: email,
			from: conf.from_email || "noreply@trustnote.org",
			subject: subject || "Trustnote user beamed you money",
			body: text,
			htmlBody: html
		});
	});
...
function replaceInTextcoinTemplate(params, handleText){
	...
	fs.readFile(__dirname + '/email_template.html', 'utf8', function(err, template) {
		...
		var text = "Here is your link to receive " + params.amount + " " + params.asset + params.usd_amount_str + ": https://trustnote.org/#textcoin?" + params.mnemonic;
